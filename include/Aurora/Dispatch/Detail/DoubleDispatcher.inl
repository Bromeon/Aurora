...
void DoubleDispatcher<B, R>::Register(R (*globalFunction)( AURORA_REPLICATE(B,D1), AURORA_REPLICATE(B,D2) ))
{
...
void DoubleDispatcher<B, R>::Register(R (C::*memberFunction)( AURORA_REPLICATE(B,D1), AURORA_REPLICATE(B,D2) ), C& object)
{
	InternalRegister(mDerivedToBase ? mMap : mCachedMap, typeid(D1), typeid(D2),
...
R DoubleDispatcher<B, R>::Call(B arg1, B arg2) const
{
	...
	if (mDerivedToBase)
	{
		...
		for (unsigned int indirectionLevel = 1; indirectionLevel <= indirectionMaxLevel; ++indirectionLevel)
		{
			...
			AURORA_CONST_FOREACH(std::vector<detail::IndirectionPair>, indirections, itr)
			{
